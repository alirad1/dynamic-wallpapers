...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${cx}" y="${cy - numberSize * 0.05}" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="${numberSize}" font-weight="700" fill="${colors.fg}">${daysLabel}</text>
  <text x="${cx}" y="${cy + labelSize * 1.4}" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="${labelSize}" fill="${colors.muted}">${escapeXml(unitLabel)}</text>
  <text x="${cx}" y="${cy + radius + goalSize * 2.2}" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="${goalSize}" font-weight="600" fill="${colors.fg}">${escapeXml(goal.slice(0, 48))}</text>
  <text x="${cx}" y="${cy + radius + goalSize * 3.4}" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="${mutedSize}" fill="${colors.muted}">${escapeXml(formatShortDate(goalDate))} · ${Math.round(progress * 100)}%</text>
</svg>`;
}
